...
public class jobsDAO {
    private static final String URL = "jdbc:sqlite:C:\\Users\\dev\\IdeaProjects\\project1_java\\src\\main\\java\\Day4\\hr.db";
    ...
    public void INSERT_jobs(jobs j) throws SQLException {
    ...
    public void UPDATE_jobs(jobs j) throws SQLException {
        Connection conn = DriverManager.getConnection(URL);
    ...
    public void DELETE_jobs(int job_id) throws SQLException {
        Connection conn = DriverManager.getConnection(URL);
        PreparedStatement st = conn.prepareStatement(DELETE_jobs);
        st.setInt(1, job_id);
        st.executeUpdate();
    }
    public jobs SELECT_ONE_id_job(int job_id) throws SQLException {
        Connection conn = DriverManager.getConnection(URL);
